...
# MIT LICENSE
# https://github.com/bartekd97/gdDelaunay
# ==== CLASSES ====
class Edge:
	var a:Vector2
	var b:Vector2
	...
	var length:float:
		get:
			if _length == 0:_length = get_length()
			return _length
		set(val):return
...
class Triangle:
	...
	func print():
		print("Triangle:%s to %s to %s" % [a,b,c])
...
static func get_adjacencies_from_triangulation(triangulation:Array[Triangle]) -> Dictionary[Vector2i, Dictionary]:
	var neighbors_by_point:Dictionary[Vector2i, Dictionary]
	
	neighbors_by_point.clear()

	for current_triangle:Triangle in triangulation:
		...
		for primary_point_index:int in range(triangle_points.size()):
			var primary_point:Vector2 = triangle_points[primary_point_index]
			var primary_point_key:Vector2i = Vector2i(primary_point)

			if !neighbors_by_point.has(primary_point_key):
				neighbors_by_point[primary_point_key] = {}

			var adjacenct_edges_of_point:Dictionary = neighbors_by_point[primary_point_key]
			
			for secondary_point_index:int in range(triangle_points.size()):
				if primary_point_index == secondary_point_index:
					continue
				var secondary_point:Vector2 = triangle_points[secondary_point_index]
				var connecting_edge:Edge = Edge.new(primary_point, secondary_point)
				var connecting_edge_key:String = edge_key(connecting_edge)

				adjacenct_edges_of_point[connecting_edge_key] = connecting_edge
...
func remove_border_triangles(triangles:Array[Triangle]) -> void:
	var sans_borders:Array[Triangle] = triangles.filter(func(tri):return !is_border_triangle(tri))
	triangles.clear()
	triangles.append_array(sans_borders)
...
## Edge_Key used in Prim's Algorithm for finding Minimum Spanning Tree
static func edge_key(edge:Delaunay.Edge) -> String:
		if edge.a < edge.b:
			return "%s|%s" % [edge.a, edge.b]
		else:
			return "%s|%s" % [edge.b, edge.a]
